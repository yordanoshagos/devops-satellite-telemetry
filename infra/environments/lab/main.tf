...
resource "aws_ecr_repository" "ground_station_api" {
  name                 = "devops-g10-ground-station-api"
...
resource "aws_ecr_repository" "telemetry_parser" {
  name                 = "devops-g10-telemetry-parser"
...
resource "aws_ecr_repository" "anomaly_detector" {
  name                 = "devops-g10-anomaly-detector"
  image_tag_mutability = "IMMUTABLE"
  force_delete         = true
...
resource "aws_vpc_security_group_ingress_rule" "callback_c_to_a" {
  ...
  from_port                    = 3001
  to_port                      = 3001
  description                  = "Callback C -> A on 3001 (service C /callback to service A)"
  tags                         = var.tags
}
